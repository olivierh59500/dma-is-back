...
go run github.com/hajimehoshi/ebiten/v2/cmd/ebitenmobile@v2.9.11 \
    ...
    -androidapi 23 \
    -javapkg com.olivierh.dmaisback \
    -o android/app/libs/dmaisback.aar \
    ./mobile
...
device_abi=$("$adb_path" shell getprop ro.product.cpu.abi | tr -d '\r')
if [ "$device_abi" != "arm64-v8a" ]; then
    echo "L’APK cible arm64-v8a, mais l’appareil annonce $device_abi." >&2
    exit 1
fi
...
echo "→ Lancement de DMA is Back!"
"$adb_path" shell am force-stop com.olivierh.dmaisback
"$adb_path" shell am start -n com.olivierh.dmaisback/.MainActivity
